...
export function getRoleBasedRedirect(role: UserRole, value?: string): string {
  const roleRedirects = {
    ...
    super_admin: '/membership-cards'
  };

  const base = roleRedirects[role] || '/dashboard';

  // ✅ Member uses barcode instead of ID
  if (role === 'member') {
    return value ? `${base}/${value}` : base;
  }

  // 🔹 Other roles still use ID normally
  return value ? `${base}/${value}` : base;
}
