...
import type { EditInvoiceFormData, CreateInvoiceItem } from "../types";
...
interface Props {
  ...
  fmt: (n: number) => string;
  onClose: () => void;
  onSubmit: (form: EditInvoiceFormData) => Promise<boolean>;
}

const EditInvoiceModal: React.FC<Props> = ({
  open, invoice, users, updatingId, fmt, onClose, onSubmit,
}) => {
  const [form, setForm] = useState<EditInvoiceFormData>({
    _id: undefined, userId: "", amount: 0,
    issueDate: new Date().toISOString().slice(0, 10),
    dueDate: "", status: "pending", items: [], notes: "", paidAmount: undefined,
  });
  ...
  useEffect(() => {
    if (invoice) {
      setForm({
        ...
        items: invoice.items || [],
        notes: invoice.notes || "",
        paidAmount: typeof invoice.paidAmount === "number" ? invoice.paidAmount : undefined,
      });
    }
  }, [invoice]);
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="flex flex-col">
                ...
                <select className="px-3 py-2 rounded border dark:bg-gray-800 dark:border-gray-600"
                  value={form.userId} onChange={(e) => setForm((p) => ({ ...p, userId: e.target.value }))} required>
              ...
              <div className="flex flex-col">
                ...
                <input type="number" className="px-3 py-2 rounded border dark:bg-gray-800 dark:border-gray-600"
                  ...
                  readOnly={form.items && form.items.length > 0} required />
            ...
            <div className="flex flex-col">
              ...
              <button type="button" className="px-4 py-2 rounded bg-gray-600 text-white hover:bg-gray-700 w-fit"
                ...
                إدارة العناصر ({form.items?.length || 0})
              </button>
              {form.items?.length > 0 && (
            ...
            <div className="flex justify-end gap-3 pt-4 border-t border-gray-200 dark:border-gray-700">
              ...
              <button type="submit" disabled={!!updatingId} className="px-4 py-2 bg-green-600 text-white rounded hover:bg-green-700 disabled:opacity-50">
                {updatingId ? "جارٍ الحفظ..." : "حفظ التعديلات"}
              </button>
            </div>
          </form>
        </div>
